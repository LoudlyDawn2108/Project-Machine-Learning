...
class MyDecisionTreeRegressor:
    ...
    def __init__(self, max_depth=100, min_samples_split=2, min_samples_leaf=1):
        ...
        self.min_samples_split = min_samples_split  # Số samples tối thiểu để chia
        self.min_samples_leaf = min_samples_leaf  # Số samples tối thiểu ở lá
        self.tree = None  # Root node của cây

    def fit(self, X, y):
        """Build cây bằng recursive splitting"""
        X = np.array(X) if not isinstance(X, np.ndarray) else X  # Chuyển về numpy array
        y = np.array(y).flatten() if not isinstance(y, np.ndarray) else y.flatten()  # Flatten y thành 1D
